/*
 * Copyright (C) 2018 Victor Toso <me@victortoso.com>
 *               2018 Yi-Soo An <yisooan@gmail.com>
 *
...
static void
test_grl_config (void)
{
  g_autoptr (GrlConfig) val = grl_config_new ("Youtube", NULL);
  g_assert (val != NULL);
}
...
static void
test_grl_registry (void)
{
  g_autoptr (GrlRegistry) val = grl_registry_get_default ();
  g_assert (val != NULL);
}
      ...
      char *argv[])
{
  ...
  g_test_add_func ("/autoptr/grl_config", test_grl_config);
  ...
  g_test_add_func ("/autoptr/grl_registry", test_grl_registry);
