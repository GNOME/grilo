...
#undef G_LOG_DOMAIN
#define G_LOG_DOMAIN "ms-log"

#define MS_DEFAULT_LOG_SPEC (G_LOG_LEVEL_WARNING | \
			     G_LOG_LEVEL_CRITICAL | \
			     G_LOG_LEVEL_ERROR)

static void
ms_log_handler (const gchar *domain,
...
setup_log_domains (const gchar *domains)
{
  ...
  g_log_set_default_handler (ms_log_handler,
			     GINT_TO_POINTER (MS_DEFAULT_LOG_SPEC));
  ...
  env_log = g_getenv ("MS_LOG");
  if (env_log) {
    domains = env_log;
    g_debug ("Using log configuration from $MS_LOG");
  }
  ...
  while (*pair) {
	...
	g_log_set_default_handler (ms_log_handler, GINT_TO_POINTER (levels));
      } else {
	g_log_set_handler (domain,
			   G_LOG_LEVEL_MASK | G_LOG_FLAG_RECURSION | G_LOG_FLAG_FATAL,
			   ms_log_handler, GINT_TO_POINTER (levels));
      }
    } else {
...
void
ms_log_init (const gchar *domains)
{
