...
grl_data_set_for_id (GrlData *data, const gchar *key_name, const GValue *value)
{
  ...
  g_return_if_fail (is_canonical (key_name), NULL);
...
grl_data_add_for_id (GrlData *data, const gchar *key_name, const GValue *value)
{
  ...
  if (key_id != GRL_METADATA_KEY_INVALID) {
    ...
    case G_TYPE_INT:
      key_id = grl_registry_register_metadata_key_for_type (registry, key_name, G_TYPE_INT);
      grl_data_add_int (data, key_id, g_value_get_int (value));
      break;

    case G_TYPE_INT64:
      key_id = grl_registry_register_metadata_key_for_type (registry, key_name, G_TYPE_INT64);
      grl_data_add_int64 (data, key_id, g_value_get_int64 (value));
      break;

    case G_TYPE_STRING:
      key_id = grl_registry_register_metadata_key_for_type (registry, key_name, G_TYPE_STRING);
      grl_data_add_string (data, key_id, g_value_get_string (value));
      break;

    case G_TYPE_FLOAT:
      key_id = grl_registry_register_metadata_key_for_type (registry, key_name, G_TYPE_FLOAT);
      grl_data_add_float (data, key_id, g_value_get_float (value));
      break;
