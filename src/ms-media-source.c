...
static void
ms_media_source_browse_full_resolution_done_cb (MsMetadataSource *source,
						MsContent *media,
						gpointer user_data,
						const GError *error)
{
                                               ...
                                               const GError *error)
{
  ...
  while (iter) {
    ...
    ms_metadata_source_resolve (map->source, 
			     map->keys, 
			     media, 
			     ms_media_source_browse_full_resolution_done_cb,
			     done_info);
...
static void
setup_full_resolution_mode (MsMediaSource *source,
			    const GList *keys,
			    MsMediaSourceResultCb user_callback,
			    gpointer user_data,
			    GList **_keys,
			...
			gpointer user_data)
{
  ...
  if (flags & MS_METADATA_RESOLUTION_FULL) {
    ...
    setup_full_resolution_mode (source,
				keys,
				callback,
				user_data,
				&_keys,
